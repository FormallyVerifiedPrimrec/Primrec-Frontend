...
export function FunctionsPanel({
  ...
  const byName = useMemo(() => new Map(functions.map((fn) => [fn.name, fn])), [functions])
  ...
  const primrecParents = useMemo(() => {
    ...
    for (const def of parseResult.ast.definitions) {
      if (def.kind === 'FunctionDefinition' && def.body.kind === 'PrimRec') {
        parents.push({ name: def.name, base: def.body.base, step: def.body.step })
      }
    }
    return parents
  ...
  const { hiddenChildren, parentsByChild, parentNames } = useMemo(() => {
    ...
    return { hiddenChildren: hidden, parentsByChild: byChild, parentNames: parents }
  }, [primrecParents])
  ...
  const listModel = useMemo(() => {
    ...
    const rows: Array<
      ...
      | { kind: 'child'; role: PrimrecChildRole; fn: DisplayFn; parentName: string }
    > = []
    ...
    for (const fn of root) {
      ...
      const matchesParent = !needle || parentDisplay.name.toLowerCase().includes(needle)
      ...
      const includeParent = matchesParent || !!(childMatches && (childMatches.base || childMatches.step))
      ...
      const shouldExpand =
        ...
        (!!needle && !!childMatches && (childMatches.base || childMatches.step))
      if (!shouldExpand) continue

      const baseDisplay = displayOf(parent.base, byName, signatures)
      const stepDisplay = displayOf(parent.step, byName, signatures)

      if (!needle || baseDisplay.name.toLowerCase().includes(needle) || matchesParent) {
        rows.push({ kind: 'child', role: 'base', fn: baseDisplay, parentName: parent.name })
      }
      if (!needle || stepDisplay.name.toLowerCase().includes(needle) || matchesParent) {
        rows.push({ kind: 'child', role: 'step', fn: stepDisplay, parentName: parent.name })
      }
    }

    return rows
  }, [byName, expanded, forcedExpandedParents, functions, hiddenChildren, parentNames, parentsByName, q, signatures])
        ...
        {listModel.length === 0 ? (
          ...
          listModel.map((row) => {
            if (row.kind === 'child') {
              ...
              return (
                <div key={`${row.parentName}:${row.role}:${row.fn.name}`} className="listItemRow" role="listitem">
                  <span className="disclosureSpacer" aria-hidden="true" />
                  <button
                    className={active ? 'listItem active listItemChild' : 'listItem listItemChild'}
                  ...
                  >
                    ...
                    {row.fn.locationLine ? <div className="fnLoc">L{row.fn.locationLine}</div> : null}
                  </button>
                </div>
              )
            }
            ...
            const isOpen = !!(parent && (expanded[parent.name] || forcedExpandedParents?.has(parent.name)))
              ...
              <div key={fn.name} className="listItemRow" role="listitem">
                {isExpandable ? (
                  <button
                    ...
                    aria-label={isOpen ? `Collapse ${fn.name}` : `Expand ${fn.name}`}
                ...
                >
                  ...
                  {fn.locationLine ? <div className="fnLoc">L{fn.locationLine}</div> : null}
                </button>
              </div>
            )
          })
