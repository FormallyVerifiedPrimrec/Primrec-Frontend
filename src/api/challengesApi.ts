...
import type { Challenge, User, Solution } from "../features/challenges/types";
...
export async function saveSubmission(
  id: string,
  payload: { success: boolean; source?: string },
): Promise<void> {
...
export async function getSolutions(
  ...
  if (params?.limit != null) qs.set("limit", String(params.limit));
  if (params?.offset != null) qs.set("offset", String(params.offset));
  return apiFetch<Solution[]>(
    `/api/challenges/${encodeURIComponent(challengeId)}/solutions?${qs.toString()}`,
  );
}
